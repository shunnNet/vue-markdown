  ...
  if (node.type === 'element') {
    ...
    switch (node.tagName) {
      ...
      case 'h6':
        ...
        break;
      case 'pre': {
        const codeChild = node.children.find(child => child.type === 'element' && child.tagName === 'code') as Element | undefined;
        if (codeChild) {
          vnodeProps.languageOriginal = Array.isArray(codeChild.properties?.className)
            ? (codeChild.properties.className as string[]).find(cls => cls.startsWith('language-'))
            : '';
